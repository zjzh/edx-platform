...
def dump_course_to_neo4j(course_key_string):
    """
...
class ModuleStoreSerializer:
    ...
    def dump_courses_to_neo4j(self, override_cache=False):
        """
...
def authenticate_and_create_graph():
    """
    ...
    """
    graph = Graph(
        protocol=COURSEGRAPH_CONNECTION.get("protocol"),
        password=COURSEGRAPH_CONNECTION.get("password"),
        user=COURSEGRAPH_CONNECTION.get("user"),
        address=COURSEGRAPH_CONNECTION.get("address"),
        port=COURSEGRAPH_CONNECTION.get("port"),
        secure=COURSEGRAPH_CONNECTION.get("secure"),
    )
    return graph
